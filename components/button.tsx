...
function ButtonDefault({ title }: Button) {
  return (
    <button
      ...
      className={
        title === "Sign in"
          ? "btn text-white bg-black focus:ring-purple-200 hover:bg-primary-opacity w-full rounded-lg btn-default"
          : "btn text-white bg-black focus:ring-purple-200 hover:bg-primary-opacity w-full rounded-lg btn-default"
      }
    >
      {title}
...
function ButtonLoadding({ title }: any) {
  return (
    <div>
      <button
        disabled
        className="btn text-white bg-black  focus:ring-purple-200 w-full btn-default rounded-lg"
      >
        <svg
          role="status"
...
function ButtonChangeImageAndVideo({
  ...
  return (
    <div className="absolute bottom-0 right-0" style={{ zIndex: 200 }}>
      <label htmlFor={title} className="relative">
        <div className="text-white bg-primary focus:ring-green-200  focus:ring-4 font-medium rounded-lg text-sm px-8 py-3 text-center sm:w-auto cursor-pointer">
          <p>{title}</p>
        </div>
      </label>
      <input
